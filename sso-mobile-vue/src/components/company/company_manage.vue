<template>
  <f7-page name="company_manage" ptr @ptr:refresh="refresh" infinite :infinite-distance="50"
    :infinite-preloader="loading" @infinite="loadMore">
    ...
    <f7-list media-list>
      <f7-list-item v-for="item in datas" :link="'/companyupdate/' + item.Id" :title="item.Name"
...
export default {
  ...
  created() {
    this.$eventbus.off("companyupdate").on("companyupdate", this.companyUpdate);
    this.$eventbus.off("companyadd").on("companyadd", this.companyAdd);
  },
  methods: {
    getQuerystring() {
      var url =
