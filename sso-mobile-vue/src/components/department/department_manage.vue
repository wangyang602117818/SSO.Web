<template>
  <f7-page name="departmentmanage" ptr @ptr:refresh="getDepartment">
    ...
    <f7-list media-list>
      <f7-list-item v-for="item in dataList" :title="item.title" :subtitle="item.desc || ' '" :key="item.key"
        :link="'/departmentupdate/' + companyCode + '/' + item.key" swipeout @swipeout:delete="delDeptartment(item.id)">
        <template #media v-if="item.layer > 0">
          <f7-icon v-for="layer in item.layer" :key="layer" width="7">·</f7-icon>
        </template>
        <f7-swipeout-actions right>
          <f7-swipeout-button color="blue" @click="addSubDepartment(companyCode, item.key)">{{
              $t("manage.add_sub_department")
          }}</f7-swipeout-button>
...
export default {
  ...
  created() {
    this.getDepartment();
    this.$eventbus.off("departentrefresh").on("departentrefresh", this.getDepartment);
  },
  methods: {
    generateList(list) {
