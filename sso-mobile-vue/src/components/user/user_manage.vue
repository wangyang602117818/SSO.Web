<template>
  <f7-page name="user_manage" ptr @ptr:refresh="refresh" infinite :infinite-distance="50" :infinite-preloader="loading"
    @infinite="loadMore">
    ...
    <f7-list media-list>
      <f7-list-item v-for="item in datas" :link="'/userupdate/' + item.UserId" :title="item.UserName"
        :subtitle="item.CompanyName + '|' + item.DepartmentName" :key="item.Id" swipeout
        ...
        <f7-swipeout-actions right>
          <f7-swipeout-button delete color="red" :confirm-title="$t('common.tips')"
            :confirm-text="$t('confirm.sure_delete')">Delete</f7-swipeout-button>
        </f7-swipeout-actions>
      </f7-list-item>
...
export default {
  ...
  created() {
    this.$eventbus.off("userupdate").on("userupdate", this.userUpdate);
    this.$eventbus.off("useradd").on("useradd", this.userAdd);
  },
  methods: {
    getQuerystring() {
      var url =
